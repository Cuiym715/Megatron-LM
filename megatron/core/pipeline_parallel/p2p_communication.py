...
def _p2p_ops(*,
    ...
    # forward
    forward_group = group
    ops.append(lambda: torch.distributed.isend(
                            tensor=tensor_send_next,
                            dst=get_pipeline_model_parallel_next_rank(),
                            group=forward_group,
                        ) if tensor_send_next is not None else None)
    ops.append(lambda: torch.distributed.irecv(
                            tensor=tensor_recv_prev,
                            src=get_pipeline_model_parallel_prev_rank(),
                            group=forward_group,
                        ) if tensor_recv_prev is not None else None)
    # backward
    backward_group = group # torch.distributed.group.WORLD
    ops.append(lambda: torch.distributed.isend(
                            tensor=tensor_send_prev,
                            dst=get_pipeline_model_parallel_prev_rank(),
                            group=backward_group,
                        ) if tensor_send_prev is not None else None)
    ops.append(lambda: torch.distributed.irecv(
                            tensor=tensor_recv_next,
                            src=get_pipeline_model_parallel_next_rank(),
                            group=backward_group,
                        ) if tensor_recv_next is not None else None)
    # even rank:    fwd_recv/fwd_send, bwd_send/bwd_recv
    # odd rank:     fwd_send/fwd_recv, bwd_recv/bwd_send
    pivot = (get_pipeline_model_parallel_rank() % 2) * 2
    ops[pivot], ops[pivot + 1] = ops[pivot + 1], ops[pivot]
    reqs = [op() for op in ops]
    reqs[pivot], reqs[pivot + 1] = reqs[pivot + 1], reqs[pivot]
