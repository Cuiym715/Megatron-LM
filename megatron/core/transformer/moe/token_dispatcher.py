...
class MoEAlltoAllTokenDispatcher(MoETokenDispatcher):
    ...
    def preprocess(self, indices: torch.Tensor) -> torch.Tensor:
        ...
        elif self.config.moe_router_force_load_balancing:
        ...
        if ep_size > 1:
            ...
            # ===================================================
            if not self.config.moe_router_force_load_balancing:
                self.input_splits = (
        ...
        if self.num_local_experts > 1:
            if not self.drop_and_pad:
                global_input_tokens, self.reversed_global_input_permutation_mapping = permute(
                    global_input_tokens, self.global_input_tokens_local_experts_indices.view(-1, 1)
                )
            else:
