...
from megatron.core.context_parallel.mappings import reduce_from_context_parallel_region
...
def switch_load_balancing_loss_func(
    ...
    """
    num_tokens = probs.shape[0] * parallel_state.get_tensor_model_parallel_world_size() * parallel_state.get_context_parallel_world_size() * topk
    num_experts = probs.shape[1]

    probs_mean_per_expert = probs.mean(dim=0)
    probs_mean_per_expert = reduce_from_tensor_model_parallel_region(probs_mean_per_expert) / parallel_state.get_tensor_model_parallel_world_size()
    probs_mean_per_expert = reduce_from_context_parallel_region(probs_mean_per_expert) / parallel_state.get_context_parallel_world_size()
    tokens_per_expert = reduce_from_tensor_model_parallel_region(tokens_per_expert)
    tokens_per_expert = reduce_from_context_parallel_region(tokens_per_expert)
...
class MoEAuxLossAutoScaler(torch.autograd.Function):
    ...
    def backward(ctx, grad_output: torch.Tensor):
        ...
        scaled_aux_loss_grad = torch.ones_like(aux_loss) * aux_loss_backward_scale.to(aux_loss.device)
...
def permute(tokens, indices, num_out_tokens: int = None, padded_mode: bool = False):
    ...
    if te_permute is not None and num_out_tokens is None:
        te_permute_output, row_id_map = te_permute(tokens, indices, -1)
        return te_permute_output, row_id_map
...
def unpermute(
    ...
    if te_permute is not None:
        output = te_unpermute(permuted_tokens, sorted_indices, probs)
        return output
...
def aggregate_aux_losses_tracker_across_pipeline_parallel(num_layers):
    """Sum aux losses across PP."""
    if len(parallel_state._MOE_AUX_LOSSES_LOGGING_TRACKER) >= 2:
        raise NotImplementedError("multiple aux_loss names")
    if len(parallel_state._MOE_AUX_LOSSES_LOGGING_TRACKER) == 0:
        parallel_state._MOE_AUX_LOSSES_LOGGING_TRACKER["moe_aux_loss"] = torch.zeros(num_layers, device="cuda")
    for name in parallel_state._MOE_AUX_LOSSES_LOGGING_TRACKER:
...
def track_moe_metrics(
    loss_scale, num_layers, moe_layer_interval, moe_first, iteration, writer, wandb_writer=None, total_loss_dict=None, per_layer_logging=False
):
    # Aux loss logging
    aggregate_aux_losses_tracker_across_pipeline_parallel(num_layers)
    aux_losses = {k: v.float() * loss_scale for k, v in get_aux_losses_tracker().items()}
    for name, loss_list in aux_losses.items():
        torch.distributed.all_reduce(loss_list)
        loss_list.div_(torch.distributed.get_world_size())
    if total_loss_dict is not None or writer is not None or wandb_writer:
        for name, loss_list in aux_losses.items():
            moe_layer_indices = torch.arange(0 if moe_first else moe_layer_interval - 1, num_layers, moe_layer_interval)
            if total_loss_dict is not None:
                if name not in total_loss_dict:
                    total_loss_dict[name] = loss_list[moe_layer_indices].mean()
                else:
                    total_loss_dict[name] += loss_list[moe_layer_indices].mean()
            ...
            if writer is not None:
                writer.add_scalar(name, loss_list[moe_layer_indices].mean(), iteration)
            ...
            if wandb_writer:
                wandb_writer.log({f"{name}": loss_list[moe_layer_indices].mean()}, iteration)
                if per_layer_logging:
                    wandb_writer.log(
                        {
