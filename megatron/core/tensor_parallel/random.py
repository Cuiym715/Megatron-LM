...
def get_during_recomputing():
    global _DURING_RECOMPUTING
    return _DURING_RECOMPUTING
...
class CheckpointFunction(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, run_function, distribute_saved_activations, kv_cache, *args):
        ctx.run_function = run_function
        ctx.distribute_saved_activations = distribute_saved_activations
        ctx.kv_cache = kv_cache and kv_cache.copy()
        ...
        with torch.no_grad():
            outputs, layer_number = run_function(kv_cache, *args)
        ...
        return outputs, *kv_cache.dump(layer_number)
    ...
    def backward(ctx, *args):
        ...
        kv_cache = ctx.kv_cache; del ctx.kv_cache
        with torch.enable_grad():
            global _DURING_RECOMPUTING
            orig_DURING_RECOMPUTING = _DURING_RECOMPUTING
            _DURING_RECOMPUTING = True
            outputs, layer_number = ctx.run_function(kv_cache, *detached_inputs)
            _DURING_RECOMPUTING = orig_DURING_RECOMPUTING
        ...
        outputs += tuple(kv_cache.dump(layer_number))
        # filter out non tensor outputs for backward pass
        assert len(outputs) == len(args), (len(outputs), len(args))
...
def checkpoint(function, distribute_saved_activations, kv_cache, *args):
    """Checkpoint a model or part of the model.
    This has been directly copied from torch.utils.checkpoint."""
    return CheckpointFunction.apply(function, distribute_saved_activations, kv_cache, *args)
